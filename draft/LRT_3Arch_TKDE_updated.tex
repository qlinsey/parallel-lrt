...
\IEEEcompsocitemizethanks{\IEEEcompsocthanksitem }% <-this % stops a space
\thanks{}}
...
\includegraphics[width=0.4\linewidth]{figtkde/example12}
...
\begin{flalign*}
&\mathbb N = \{0..n-1\}               & \\
&\mathbb R = \{0..\lfloor \frac{n+1}{2}\rfloor -1\} \times \{0..n\}     &\\
&\mathbb T = \{(i,j) \in \mathbb N^2 | i<=j\}   &\\
&\mathbb M = \{0.. \lfloor \frac{n+1}{2}\rfloor \times (n+1)-1\}  &\\
\end{flalign*}
...
&\mathbb R_{1,2} = \{0..\lfloor \frac{n_{1,2}+1}{2}\rfloor-1\} \times \{0..n_{1,2}\}&\\
&\mathbb T_{1,2} = \{(a_{1,2},b_{1,2}) \in \mathbb N_{1,2}^2 | a_i<=b_{1,2}\}&\\
&\mathbb M_{1,2} = \{0.. \lfloor \frac{n_{1,2}+1}{2}\rfloor \times (n_{1,2}+1)-1\}&\\
\end{flalign*}
...
\begin{flalign*}
&\mathbb N_k = \{0..n_k-1\}          & \\
&\mathbb R_k = \{0..\lfloor \frac{n_k+1}{2}\rfloor-1\} \times \{0..n_k\}&\\
&\mathbb T_k = \{(a_k,b_k) \in \mathbb N_k^2 | a_i<=b_k\}&\\
&\mathbb M_k = \{0.. \lfloor \frac{n_k+1}{2}\rfloor \times (n_k+1)-1\}&\\
\end{flalign*}
...
\subsubsection{Naive vs. Inclusive/Exclusive Implementation}
 ...
 \begin{comment}
  The naive approach requires $\bigo (n^2)$ computation  for a given region in the worst case. The computation result of each region is stored into memory and $kbest$ regions are obtained by sorting all of the results at the end. The final top-k results takes up $\bigo(n^4)$ space. Due to large memory space required and long computing, the naive approach was only be able to generate the $kbest$ results from $(100,100)$ to $(300,300)$ in our machine. \\
  Inclusive/Exclusive pre-computation approach is devised to improve the computation efficiency, which reduces the computation for each region to $\bigo (1)$. In this approach, pre-computed sets take up $\bigo (4n^2)$ space and only the $kbest$ results kept in heap structure at the end, therefore the total space complexity is $\bigo (n^2)$.
 \end{comment}
...
\includegraphics[width=0.95\linewidth]{figtkde/multicore}
...
\includegraphics[width=0.85\linewidth]{figtkde/norec_multicore}
...
~~\subfloat[]{
\includegraphics[width=0.95\linewidth]{figtkde/gpuspeff}
\label{gpu:subfig3}
}\\
~~\subfloat[]{
\includegraphics[width=0.9\linewidth]{figtkde/norec_gpu}
\label{gpu:subfig4}
}
