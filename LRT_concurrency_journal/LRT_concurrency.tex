...
In this section, we use an example to demonstrate two different parallel strategies for performing LRT statistics in one-dimension data grid. The strategies are (i) \emph{Overlapping Enumeration}, and (ii) \emph{Non-overlapping Enumeration}. The Overlapping Enumeration strategy has been implemented in~\cite{apweb}. Here we simulate the architecture environment in ~\cite{apweb} and illustrate its drawbacks. The non-overlapping Enumeration strategy, called Range Mapping scheme, will be presented in this paper to overcome the limitations of the Overlapping Enumeration strategy. For simplicity, we will describe the strategies on abstract parallel machine not detailing its implementation on a concrete parallel platform. \\


%BFS: Please expand the example text. The explanation is too short!! Highlight the difference between the two strategies.

EXAMPLE: Assume we have a one-dimensional spatial data grid with  five data points, denoted as $\{O_0,O_1,O_2,O_3,O_4\}$. We would like to compute the LRT for the one-dimensional spatial data grid. In this example, we assume that each interval in data grid contains $(m_r, b_r)$ to be used for $LRT$ computation. $m_r$ and $b_r$ takes 4 bytes separately. Each parallel unit of the parallel machine can only maximally process 16 bytes of data. Therefore, each parallel unit can only maximally process $2$ intervals.\\
 To compute the LRT, we need to enumerate all of the intervals among these five points. We express $l_{(i,j)}$ as an interval between point $O_i$ and $O_j$, where $ (0 \leq i <j \leq n)$.  There are ${\frac{4 \cdot (4+1)}{2}}=2\times 5=10$ pairs for $(i,j)$ fulfilling  the constraint.

 \begin{itemize}
 \item \emph{Overlapping Enumeration parallel strategy:}
Since each parallel unit of the parallel machine can only maximally store and process $16$ bytes of data, only $2$ intervals can be processed for a single parallel unit. The overlapping strategy is proposed to enumerate every two continuous overlapped intervals (i.e. intervals $(i,j),(i,j+1)$ fitted to a parallel unit. The rest of non-continuous intervals can not be fitted to a parallel unit because the data size to calculate $LRT$ is larger than $16$ bytes. The $LRTs$ of these non-continuous intervals will be processed and merged on a sequential unit (CPU) (please see the details in ~\cite{apweb}). To allocate statistic values of intervals to parallel unit for processing $LRT$ calculation, every two consecutive intervals are assigned to a certain parallel unit according to the starting point index of interval. Table ~\ref{exampletable} shows the intervals processed by each parallel unit. For example, $PU_0$ contains intervals: $\{l_{(0,0)},l_{(0,1)}\}$ ; $PU_1$ contains intervals: $\{l_{(1,1)},l_{(1,2)}\}$, we can see that the overlapping scheme enumerates all of the intervals up to size of two. Figure~\ref{example1} illustrates the enumeration of all the intervals for those $5$ data points. \\
 There are few drawbacks of this approach: (1) Non-continuous intervals cannot be processed by parallel unit. There are around $n\times {n \choose k}$ intervals will be processed sequentially. (2)Some $PUs$ may have fewer number of intervals in this way. For example, $PU_4$ has only one interval(i.e. $\{l_{(3,3)}\}$). This leads to an observation of workload unbalancing among $PUs$.   (3)Furthermore, for each $PU$, each interval is assigned to a thread. The lengths of intervals are different and this can further creates different workload among different threads when accumulating statistics when computing $LRT$.\\
 To overcome these drawbacks and to fully utilize the parallel unit, Non-Overlapping Enumeration parallel strategy and pre-computing strategy are proposed in the following to enable $LRT$ computation of all of intervals by each parallel unit.  Moreover, horner scheme is used to allocate certain interval to certain $PU$.
