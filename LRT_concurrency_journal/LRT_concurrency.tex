...
%BFS: Please check largest grid they computed with simple method.
...
In this paper, we propose a parallel processing scheme for LRT computations for one parameter exponential (1EXP) distribution for multi-dimensional spatial data grids. The parallel processing scheme reduces LRT computations to a completely balanced embarrassingly parallel problem utilizing a novel range mapping scheme. The new range mapping scheme relies on a dynamic pre-computation algorithm. Experiments were conducted for various computer architectures and platforms including shared-memory multi-core machines, multi-GPGPU computers, and the EC2 cloud computer. Extensive experiments are provided to demonstrate the utility of this approach and extensive performance analysis is given.
%BFS: Mention speedups here to single-core CPU.
...
% Remove notion of GPGPU in this section / just say parallel computer
...
%BFS: Please expand the example text. The explanation is too short!! Highlight the difference between the two strategies.
...
% BFS: Explain in more detail what Honer scheme is.
...
\end{eqnarray}
where  $ N$ is the set of data points which partitions one-dimensional grid to intervals and total number of these points is $n$. $ R$ is the rectangular space formed by $(\lfloor \frac{n+1}{2}\rfloor -1, n)$ and each point $(i,j)$ (i.e. interval) in this space is assigned to a certain $PU_y$. $T$ is the triangular space formed by original intervals, where each interval is $l_(i',j')$ $(0<i'<j'\leq n)$. $ M$ is total number of  intervals for computing $LRT$.\\
 To generalize the mapping scheme when grid size $n$ is in even or odd case, we extend the calculation of total intervals from $\frac{n+1}{2} \times n$ to $\lfloor \frac{n+1}{2}\rfloor \times (n+1)$. When $n$ is even, we know $\lfloor \frac{n+1}{2}\rfloor =\frac{n}{2}$. When $n$ is odd, we know $\lfloor \frac{n+1}{2}\rfloor= \frac{n+1}{2}$. Since the number of total intervals is computed as $\frac{n+1}{2} \times n $ no matter grid size $n$ is even or odd, this generalized form calculation will have $\frac{n+1}{2}$ more intervals with enlarged set when $n$ is odd but have exact calculation when $n$ is even. \\

\noindent  \emph{(1) The grid size $n$ is even:} A mapping Function $\phi:  M \rightarrow  R \rightarrow  T$  is composed of two functions: $\phi_1:  M \rightarrow  R$ and $\phi_2:  R \rightarrow  T$. Firstly, $\phi_1$ transforms  $y\in  M$ to a pair $(i,j)\in  R$ in rectangular space. Secondly, $\phi_2$ transforms the pair $(i,j)\in  R$ to a pair $(i',j')\in  T$ in triangular space.\\
...
\end{definition}
IR (Index to rectangular) mapping function assigns a point $(i,j)$ in rectangular space $R$ to $y$ of $PU_y$  by applying inverse horner scheme, where $y\in \lfloor \frac{n+1}{2}\rfloor \times (n+1)$.  Point $(i,j)$ is computed by $i  = y / (n+1)$ and $j = y \mod (n+1)$ , where $y$ is the index of a particular $PU$ and constant value of $n$ is the total number of data points in one-dimensional grid.
 \vspace{5mm}
\begin{lemma}
Mapping function $\phi_1$ transforms $y\in M$ to $(i,j) \in R$, where $i \in \{0,...,\frac{n}{2}-1\}$ and $j \in \{0,...,n\}$.
\end{lemma}
When $n$ is even, we know ${\lfloor \frac{n+1}{2}\rfloor} = \frac{n} {2}$. Since $y\in M$ and size of $M$ is equal to $ \frac{n}{2} \times n$, it is obviously seen that $i\in (0,\frac{n}{2}-1)$ when $i  = y / (n+1)$. Similarly, $j\in (0,n)$ when $j = y \mod (n+1)$.
\vspace{5mm}
\begin{lemma}
Mapping function $\phi_1:   M \rightarrow  R, y \mapsto (i,j)$ is bijective.
\end{lemma}
 In one-dimension grid, for each $PU_y$, we can denote $y=i\times n+j$, where $y \in (0,{\frac{n \cdot (n+1)}{2}})$ and $0\leq i, j < n$. From the horner scheme and the quotient-remainder theorem , we know there is one-to-one mapping from $y$ to $(i,j)$. Correspondingly, given $(i,j)$, $y$ can be constructed as $y=i\times n+j$. Therefore, $y \mapsto (i,j)$ is bijective.
\vspace{5mm}
...
\end{eqnarray}
\end{definition}
RT (rectangular to triangular) mapping function transforms point $(i,j)$ in rectangular space to point $(i',j')$ in triangular space shown in above equation. This transformation is achieved by satisfying the constraints: $(i,j)\in \{(0,\lfloor (n+1)/2)\} \times \{(0,n)\}$ and $0\leq i' \leq j'<n$. To fulfill this constraint, $i+j=n$ divide the rectangular space into two sub-spaces: (1) $i+j<n$, (2) $i+j\geq n$. In (1), transformed point $(i',j')$ is calculated as $(i,i+j)$ . In (2), transformed point $(i',j')$ is calculated as $(n-i-1,(n-i-1)+(n-j))$ (i.e. $(n-i-1,2n-i-j-1)$). It is clearly seen that $(i',j')$ satisfies $0\leq i' \leq j'<n$ after transformation. Figure~\ref{even:subfig1} visualizes the mapping.

\vspace{5mm}
\begin{lemma}
Mapping function $\phi_2$ transform $(i,j) \in R$ to $(i',j')\in T $, where $i' \in \{0,...,n\}$ and $j' \in \{0,...,n\}$ in triangular space.
\end{lemma}
...
\end{lemma}
From above definition, we see that each $(i,j)$ has one to one mapping to $(i',j')$. There has $i'=i, j'=i+j$ when $i+j<n$. And there has $i'=n-i-1, j'=2n-i-j-1$ when $i+j\geq n$. There is no over-lapping mapped point $(i',j')$ based on the constraint of $i+j<n$. Correspondingly,  given $(i',j')$, $(i,j)$ can be obtained by $i=i', j=j'-i'$ or $i=n-i'-1, j=i'+n-j'$. Therefore, $y \mapsto (i,j)$ is bijective.
\vspace{5mm}
\begin{corollary}
...
\noindent \emph{(2) The grid size $n$ is odd:} Function $\phi:  M \rightarrow  R \rightarrow  T$ is composed of two mapping functions: $\phi_1:  M \rightarrow  R$ and $\phi_2:  R \rightarrow  T$. Firstly, $\phi_1$ transforms $y\in  M$ to a pair $(i,j)\in  R$ in rectangular space. Secondly, $\phi_2$ transforms  $(i,j)\in  R$ to a pair $(i',j')\in  T$ in triangular space.
\vspace{5mm}
\noindent\begin{definition}
IR Mapping Function
\begin{eqnarray}
...
\end{eqnarray}
 where  $i  = y / (n+1)$ and $j = y \mod (n+1)$.\\
 IR (Index to rectangular) mapping function assigns a point $(i,j)$ in rectangular space $R$ to $y$ of $PU_y$  by applying inverse horner scheme, where $y\in \lfloor \frac{n+1}{2}\rfloor \times (n+1)$.  Point $(i,j)$ is computed by $i  = y / (n+1)$ and $j = y \mod (n+1)$ , where $y$ is the index of a particular $PU$ and constant value of $n$ is the total number of data points in one-dimensional grid. Since $n$ is odd in this case, there has $\frac{n+1}{2}$ more data points (intervals) than total number of intervals and they  won't be processed.
\end{definition}
 \vspace{5mm}
\begin{lemma}
Mapping function $\phi_1$ transforms $y\in M$ to $(i,j) \in R$, where $i \in \{0,...,\frac{n+1}{2}-1\}$ and $j \in \{0,...,n\}$.
\end{lemma}
When $n$ is odd, we know $ {\lfloor \frac{n+1}{2}\rfloor} = {\frac{n+1}{2}}$.  Since $y\in M$ and size of $M$ is extend to $  \frac{n+1}{2} \times (n+1)$ , we can derive that $i\in \{0,...,\frac{n+1}{2}-1\}$ when $i  = y / (n+1)$. Similarly, $j\in (0,n)$ when $j = y \mod (n+1)$.
\vspace{5mm}
\begin{lemma}
Mapping function $\phi_1:   M \rightarrow  R,y \mapsto (i,j)$ is bijective.
\end{lemma}
 In one-dimension grid, for each $PU_y$, we can denote $y=i\times (n+1)+j$, where $y \in (0,{\frac{(n+1) \cdot (n+1)}{2}})$ and $0\leq i, j < n$. From the horner scheme and the quotient-remainder theorem , we know there is one-to-one mapping from $y$ to $(i,j)$. Correspondingly, given $(i,j)$, $y$ can be constructed as $y=i\times (n+1)+j$. Therefore, $y \mapsto (i,j)$ is bijective. 
\vspace{5mm}
\begin{definition}
Mapping Function
\begin{eqnarray}
...
\end{eqnarray}
\end{definition}
RT (rectangular to triangular) mapping function transforms point $(i,j)$ in rectangular space to point $(i',j')$ in triangular space shown in above equation. This transformation is achieved by satisfying the constraints: $(i,j)\in \{(0,\lfloor (n+1)/2)\} \times \{(0,n+1)\}$ and $0\leq i' \leq j'<n$. To fulfill this constraint, $i+j=n$ divide the rectangular space into two sub-spaces: (1) $i+j<n$, (2) $i+j\geq n$. Furthermore, when $n$ is odd, we have $\frac{(n+1)}{2}$ more points than needed and thus the rectangular shape is enlarged, the constraint of $(i+1) \leq \frac{(n+1)}{2}$ is applied to limit the unnecessary transformation of points in rectangular space to triangular space. In (1), transformed point $(i',j')$ is calculated as $(i,i+j)$ . In (2), transformed point $(i',j')$ is calculated as $(n-i-1,(n-i-1)+(n-j))$ (i.e. $(n-i-1,2n-i-j-1)$). This mapping strategy shows that $(i',j')$ satisfies $0\leq i' \leq j'<n$ after transformation. Figure~\ref{odd:subfig1} visualizes the mapping.
\vspace{5mm}
\begin{lemma}
