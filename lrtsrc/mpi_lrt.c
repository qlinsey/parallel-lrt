...
{   
   ...
   clock_t   t_compute;    //time for computing LRT

   double    p_cstart,
             p_cend,
             p_clast;
   struct    timespec ebegin, eend;
   double    elapsed;
   ...
   if (myid == 0) {
      t_compute = clock();  
      p_cstart=MPI_Wtime();
      clock_gettime(CLOCK_MONOTONIC, &ebegin);        
   }
   ...
   if (myid == 0)  {
      ...
      if (profiling_flag) {             // output time measurements and memory consumption
         ...
         elapsed = (eend.tv_nsec - ebegin.tv_nsec) / 1000000000.0;
