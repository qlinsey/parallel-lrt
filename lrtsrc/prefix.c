...
//
//  functions to compute prefix sums for computing the event counts of
//  a rectangle in O(1) time.
//  
...
//
// free prefix sums and data structure for prefix sums
// 

void free_psums(prefix_sums *p)
{
  free(p->A);
  free(p->B);
  free(p->X);
