...
//
// simple wrapper to check error conditions
//
__host__ void cudaSafe( cudaError_t err)
{
   if(err!=cudaSuccess) { 
      error("something went wrong with CUDA %s",cudaGetErrorString(err));
   }
}
                          ...
                          )
{
   ...
   // store heap_size
   grid_heapsizes[((blockIdx.y*blockDim.y+threadIdx.y)*gridDim.x*blockDim.x+blockIdx.x*blockDim.x+threadIdx.x)] = heap_size;

   // Write heap back to global memory
   memcpy(&grid_heaps[((blockIdx.y*blockDim.y+threadIdx.y)*gridDim.x*blockDim.x+blockIdx.x*blockDim.x+threadIdx.x)*kbest],r,heap_size*sizeof(rectangle));
}


//
// LRT computation on GPU
// 
__host__ rectangle *gpu_lrt(grid *g,int kbest)
{
   ...
   // measure time for kernel and memory transfer
   ...
   cudaSafe(cudaMalloc(&pa_d,num_cells));
   cudaSafe(cudaMalloc(&pb_d,num_cells));
   cudaSafe(cudaMalloc(&px_d,num_cells));
   cudaSafe(cudaMalloc(&py_d,num_cells));
   cudaSafe(cudaMemcpy(pa_d,p->A,num_cells,cudaMemcpyHostToDevice));
   cudaSafe(cudaMemcpy(pb_d,p->B,num_cells,cudaMemcpyHostToDevice));
   cudaSafe(cudaMemcpy(px_d,p->X,num_cells,cudaMemcpyHostToDevice));
   cudaSafe(cudaMemcpy(py_d,p->Y,num_cells,cudaMemcpyHostToDevice));
  
   // allocate memory for resulting heaps for each thread 
   // and heap sizes.  
   rectangle *grid_heaps_d; 
   cudaMalloc(&grid_heaps_d,sizeof(rectangle)*kbest*THREADS_IN_GRID);
   int *grid_heapsizes_d; 
   cudaMalloc(&grid_heapsizes_d,sizeof(int)*THREADS_IN_GRID);
   ...
   rectangle *grid_heaps = ALLOCV(rectangle, sizeof(rectangle)*kbest*THREADS_IN_GRID);
   int *grid_heapsizes = ALLOCV(int, sizeof(int) * THREADS_IN_GRID); 

   cudaSafe(cudaMemcpy(grid_heaps,grid_heaps_d,sizeof(rectangle)*kbest*THREADS_IN_GRID,cudaMemcpyDeviceToHost));
   cudaSafe(cudaMemcpy(grid_heapsizes,grid_heapsizes_d,sizeof(int)*THREADS_IN_GRID,cudaMemcpyDeviceToHost));

   // free cuda memory
   cudaSafe(cudaFree(pa_d));
   cudaSafe(cudaFree(pb_d));
   cudaSafe(cudaFree(px_d));
   cudaSafe(cudaFree(py_d));
   cudaSafe(cudaFree(grid_heaps_d));
   cudaSafe(cudaFree(grid_heapsizes_d));
   t_kernel = clock() - t_kernel;
   ...
   for (i=0;i<THREADS_IN_GRID;i++) { 
      int hs = grid_heapsizes[i];
      rectangle *rs = &grid_heaps[i*kbest];
      int j;
      assert(hs <= kbest && "wrong heap size");
      for (j=0;j<hs;j++) {
         if (heap_size < kbest) { 
            r[heap_size] = rs[j];
            heap_size = heap_size + 1;
   ...
   // free auxilliary data structures
   free(grid_heaps); 
   free(grid_heapsizes); 
   ...
   t_merge = clock() - t_merge;
    ...
    if (cudaprofiling_flag) {             // output time measurements and memory consumption
        fprintf(stderr,"time to execute kernel: %f (s)\n",((double)t_kernel)/CLOCKS_PER_SEC);
        fprintf(stderr,"time to merge results: %f (s)\n",((double)t_merge)/CLOCKS_PER_SEC);
    }
